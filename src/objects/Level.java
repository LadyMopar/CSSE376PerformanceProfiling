	...
	public void draw(Graphics2D g2) {
		...
		for (int r = 0; r < this.map.length; r++) {
			for (int c = 0; c < this.map[r].length; c++) {
				...
				//checking to see if currentPosition is already in the HashMap
				if (!imageCache.containsKey(r + c)) {
					imageCache.put(r+c, currentPosition);
				}
				//If the image hasn't changed, there's no point in redrawing it
				//This checks that.
				if(imageCache.get(r+c) != currentPosition){
